...
void setup() {
    Serial.begin(115200);
    delay(1000);
    LOG_BOOT("SpHEC Meter v2.8.0 Booting...");
...
void serialCalTask(void* pvParameters) {
    ...
    if (choice == 'p') {
        ...
        for (int i = 0; i < CALIBRATION_POINT_COUNT; ++i) {
            ...
            // --- FIX: Capture the live temperature along with the voltage ---
            double capturedVoltage;
            double capturedTemp;
            taskENTER_CRITICAL(&sharedDataMux);
            ...
            // --- FIX: Pass all required data to the manager ---
