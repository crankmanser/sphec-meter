...
// --- NEW: Private helper method for buffer temperature correction ---
double getTemperatureCorrectedBufferValue(double nominalValue, double temperature);


// --- Constructor & Begin (Unchanged) ---
CalibrationManager::CalibrationManager() :
    _faultHandler(nullptr),
    _initialized(false),
    _newPointsCount(0)
{}
bool CalibrationManager::begin(FaultHandler& faultHandler) {
    _faultHandler = &faultHandler;
    _initialized = true;
    return true;
}

// --- Core Measurement Methods (Unchanged) ---
double CalibrationManager::getCalibratedValue(double filteredVoltage) {
    if (!_currentModel.isCalibrated) return 0.0;
    return (_currentModel.coeff_a * std::pow(filteredVoltage, 2)) +
           (_currentModel.coeff_b * filteredVoltage) +
           _currentModel.coeff_c;
}
double CalibrationManager::getCompensatedValue(double rawValue, double measuredTemperature, bool isEC) {
    if (isEC) {
        const double ecAlpha = 0.0191;
        const double refTemp = 25.0;
        return rawValue / (1.0 + ecAlpha * (measuredTemperature - refTemp));
    } else {
        const double phAlpha = 0.003;
        const double neutralPh = 7.0;
...
void CalibrationManager::startNewCalibration() {
    _newPointsCount = 0;
    _newModel = CalibrationModel();
}

// --- MODIFIED: This now takes the actual temperature to store in the model ---
bool CalibrationManager::addCalibrationPoint(double voltage, double knownValue, double temperature) {
    ...
    // --- NEW: Use the helper to get the real pH of the buffer at the measured temp ---
    ...
    _newModel.points[_newPointsCount].value = correctedValue; // Store the corrected value
    _newPointsCount++;

    // Store the average temperature of the calibration points
    if (_newPointsCount == 1) {
        _newModel.calibrationTemperature = temperature;
    } else {
        _newModel.calibrationTemperature = (_newModel.calibrationTemperature * (_newPointsCount - 1) + temperature) / _newPointsCount;
    }
    return true;
}

// --- CalculateNewModel and other methods remain the same ---
double CalibrationManager::calculateNewModel(const CalibrationModel& previousModel) {
    ...
    if (previousModel.isCalibrated) {
        const int steps = 100;
        double v_low = std::min({x[0], x[1], x[2]});
        double v_high = std::max({x[0], x[1], x[2]});
...
// --- NEW: Buffer Temperature Compensation Logic ---
double getTemperatureCorrectedBufferValue(double nominalValue, double temperature) {
    // Standard NIST buffer temperature correction tables
    ...
    else return nominalValue; // Not a standard buffer, return nominal value
